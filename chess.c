...
int intersect_ray_with_objects(int obnum, double a[3], double c[3], double res[1])
{
	...
	else if (obtype[obnum] == 2)
	{
		...
		if (quadratic(A, B, C, r) != 0)
		{
			...
			if ((intersect[0] > 1 || intersect[0] < 0) && (intersect[1] > 1 || intersect[1] < 0))
			{
				return 0;
			}
			if (r[0] < 0 && r[1] < 0)
...
int ray(double argb[3], double start[3], double end[3], double board_pt[3])
{
	...
	for (j = 0; j < objects_intersected; j++)
	{
		...
		if (intersect_ray_with_objects(i, objStart, objEnd, res) == 0)
		{
			continue;
		}
...
int movePiece(int piece, double position[3])
{
	...
	if (pieceID[piece] == 1)
	{
		create_pawn(position[0], position[1], position[2], isPieceWhite[piece]);
	}
	if (pieceID[piece] == 2)
	{
		create_rook(position[0], position[1], position[2], isPieceWhite[piece]);
	}
	if (pieceID[piece] == 3)
	{
		create_knight(position[0], position[1], position[2], isPieceWhite[piece]);
	}
	if (pieceID[piece] == 4)
	{
		create_bishop(position[0], position[1], position[2], isPieceWhite[piece]);
	}
	if (pieceID[piece] == 5)
	{
		create_queen(position[0], position[1], position[2], isPieceWhite[piece]);
	}
	if (pieceID[piece] == 6)
	{
		create_king(position[0], position[1], position[2], isPieceWhite[piece]);
	}
...
int checkPath(double start[3], double end[3], int isWhite)
{
	...
	{
		squares = abs(diff[0] / 4);
	} // Diagonal
	else if (abs(diff[0]) > abs(diff[1]))
	{
		squares = abs(diff[0] / 4);
	} // Horizontal
	else
	{
		squares = abs(diff[1] / 4);
	} // Vertical
...
int checkPawnMoves(int piece, double position[3])
{
	...
	{
		if (checkForPieces(position) == !isPieceWhite[piece])
		{
			return 2;
		}
		else
	...
	{
		if (checkPath(piece_location[piece], position, isPieceWhite[piece]) == 1)
		{
			return 1;
		}
	}
...
int checkKnightMoves(int piece, double position[3])
{
	...
	if ((abs(positionDiff[0]) == 8 && abs(positionDiff[1]) == 4) || (abs(positionDiff[0]) == 4 && abs(positionDiff[1]) == 8))
	{
		...
		else if (check == !isPieceWhite[piece])
		{
			return 2;
		}
		else
...
int checkKingMoves(int piece, double position[3])
{
	...
	if (hasMoved[piece] == 0 && (abs(positionDiff[0]) == 8 && abs(positionDiff[1]) == 0))
	{
		if (isPieceWhite[piece])
		{
			if (positionDiff[0] == -8 && hasMoved[8] == 0 && checkPath(piece_location[piece], position, isPieceWhite[piece]))
			{
				return 3;
			}
			if (positionDiff[0] == 8 && hasMoved[15] == 0 && checkPath(piece_location[piece], position, isPieceWhite[piece]))
			{
				return 4;
			}
		}
		...
		{
			if (positionDiff[0] == -8 && hasMoved[24] == 0 && checkPath(piece_location[piece], position, isPieceWhite[piece]))
			{
				return 3;
			}
			if (positionDiff[0] == 8 && hasMoved[31] == 0 && checkPath(piece_location[piece], position, isPieceWhite[piece]))
			{
				return 4;
			}
		}
...
int castle(int piece, int side)
{
	...
	if (side == 0)
	{
		...
		if (squareInCheck(checkPosition, isPieceWhite[piece]))
		{
			return 0;
		}
...
int waitMove()
{
	...
	while (!complete)
	{
		...
		if (isPieceWhite[piece_hit] == !isWhiteTurn)
		{
			validity = 0;
		}
		if (validity == 0)
		...
		if (isWhiteTurn && willKingBeInCheck(piece_hit, new_position, isWhiteTurn))
		{
			continue;
		}
		if (!isWhiteTurn && willKingBeInCheck(piece_hit, new_position, isWhiteTurn))
...
int main()
{
	...
	num_objects = 0;
	num_pieces = 0;
	captureLocation[0][0] = 22;
	captureLocation[0][1] = 0;
	captureLocation[0][2] = -18;
	captureLocation[1][0] = -22;
	captureLocation[1][1] = 0;
	captureLocation[1][2] = 18;
	isInCheck[0] = 0;
	isInCheck[1] = 0;
